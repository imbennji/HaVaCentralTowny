	...
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException {
		if (src instanceof Player) {
			...
			if (!towny.isAdmin() && !plot.getFlag("public") && (playerTowny == null || !towny.getUUID().equals(playerTowny.getUUID()))) {
				src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_PERM_PLOTRENT));
				return CommandResult.success();
			}
			if (!plot.isForRent() || plot.isOwned()) {
				src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_PLOTNFR));
				return CommandResult.success();
			}
			...
			if (result.getResult() == ResultType.ACCOUNT_NO_FUNDS) {
				src.sendMessage(Text.builder()
						...
						.append(Utils.formatPrice(LanguageHandler.colorYellow(), price))
