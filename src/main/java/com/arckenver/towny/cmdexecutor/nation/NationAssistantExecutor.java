...
public class NationAssistantExecutor implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException {
        ...
        if (!ctx.<String>getOne("action").isPresent() || !ctx.<String>getOne("player").isPresent()) {
            src.sendMessage(Text.of(LanguageHandler.colorYellow(), "/nation assistant add <player>\n/nation assistant remove <player>"));
            return CommandResult.success();
        }
        ...
        if (targetTown == null || !targetTown.hasNation() || !nation.getUUID().equals(targetTown.getNationUUID())) {
            src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_NATION_NO_TARGET_PLAYER));
            return CommandResult.success();
        }
        ...
        if (action.equalsIgnoreCase("add")) {
            ...
            src.sendMessage(Text.of(LanguageHandler.colorGreen(), LanguageHandler.INFO_NATION_ASSISTANT_ADDED));
            ...
            DataHandler.saveNation(nation.getUUID());
            src.sendMessage(Text.of(LanguageHandler.colorGreen(), LanguageHandler.INFO_NATION_ASSISTANT_REMOVED));
        } else {
