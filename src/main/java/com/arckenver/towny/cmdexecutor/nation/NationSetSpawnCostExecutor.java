...
public class NationSetSpawnCostExecutor implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException {
        ...
        if (!ctx.<Double>getOne("cost").isPresent()) {
            src.sendMessage(Text.of(LanguageHandler.colorYellow(), "/nation setspawncost <amount>"));
            return CommandResult.success();
        }
        ...
        if (cost > max) {
            src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_NATION_SPAWN_COST.replace("{MAX}", String.valueOf(max))));
            return CommandResult.success();
        }
        ...
        src.sendMessage(Text.of(LanguageHandler.colorGreen(), LanguageHandler.INFO_NATION_SPAWN_COST));
        return CommandResult.success();
    }
}
