...
public class NationAllyExecutor implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException {
        ...
        if (!ctx.<String>getOne("action").isPresent() || !ctx.<String>getOne("nation").isPresent()) {
            src.sendMessage(Text.of(TextColors.YELLOW, "/nation ally add <nation>\n/nation ally remove <nation>"));
            return CommandResult.success();
        }
        ...
        if (action.equalsIgnoreCase("add")) {
            if (nation.getAllies().contains(target.getUUID())) {
                src.sendMessage(Text.of(TextColors.RED, LanguageHandler.ERROR_NATION_ALREADY_ALLY));
                return CommandResult.success();
            }
            ...
            src.sendMessage(Text.of(TextColors.GREEN, LanguageHandler.INFO_NATION_ALLY_ADDED));
        } else if (action.equalsIgnoreCase("remove")) {
            if (!nation.getAllies().contains(target.getUUID())) {
                src.sendMessage(Text.of(TextColors.RED, LanguageHandler.ERROR_NATION_NOT_ALLY));
                return CommandResult.success();
            }
            ...
            DataHandler.saveNation(nation.getUUID());
            DataHandler.saveNation(target.getUUID());
            src.sendMessage(Text.of(TextColors.GREEN, LanguageHandler.INFO_NATION_ALLY_REMOVED));
        } else {
