	...
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException {
		if (src instanceof Player) {
			...
			if (!ctx.<String>getOne("towny").isPresent()) {
				src.sendMessage(Text.of(LanguageHandler.colorYellow(), "/t join <towny>"));
			...
			if (req != null) {
				src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_ALREADYASKED));
				return CommandResult.success();
			}
			...
			if (towny.getFlag("open") || req != null) {
				...
				for (UUID uuid : towny.getCitizens()) {
					...
					if (optPlayer.isPresent())
						optPlayer.get().sendMessage(Text.of(LanguageHandler.colorGreen(), LanguageHandler.INFO_JOINTOWNANNOUNCE.replaceAll("\\{PLAYER\\}", guestPlayer.getName())));
				}
			...
			if (townyStaffPlayers.isEmpty()) {
				src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_NOSTAFFONLINE));
				return CommandResult.success();
			}
			...
			src.sendMessage(Text.of(LanguageHandler.colorGreen(), LanguageHandler.INFO_INVITSEND.replaceAll("\\{RECEIVER\\}", townyName)));
		} else {
