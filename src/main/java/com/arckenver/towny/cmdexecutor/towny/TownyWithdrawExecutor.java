...
public class TownyWithdrawExecutor implements CommandExecutor
{
	...
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException
	{
		if (src instanceof Player)
		{
			...
			{
				src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_NOENOUGHMONEYTOWN));
				return CommandResult.success();
			}
			...
			{
				builder
				.append(Text.of(LanguageHandler.colorGreen(), s1[0].split("\\{BALANCE\\}")[0]))
				...
				.append(Text.of(LanguageHandler.colorGreen(), s1[0].split("\\{BALANCE\\}")[1]));
			}
			builder.append(Utils.formatPrice(LanguageHandler.colorGreen(), amount));
			if (s1[1].indexOf("\\{BALANCE\\}") >= 0)
			{
				builder
				.append(Text.of(LanguageHandler.colorGreen(), s1[1].split("\\{BALANCE\\}")[0]))
				.append(Utils.formatPrice(LanguageHandler.colorGreen(), optTownyAccount.get().getBalance(TownyPlugin.getEcoService().getDefaultCurrency())))
				.append(Text.of(LanguageHandler.colorGreen(), s1[1].split("\\{BALANCE\\}")[1]));
			}
