...
public class TownyTaxesExecutor implements CommandExecutor
{
	...
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException
	{
		if (src instanceof Player)
		{
			...
			{
				src.sendMessage(Text.of(LanguageHandler.colorYellow(), "/t taxes <amount>"));
				return CommandResult.success();
			}
			double newTaxes = ctx.<Double>getOne("amount").get();
			if (!ConfigHandler.getNode("towny", "canEditTaxes").getBoolean())
			{
				src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_TAXEDIT));
				return CommandResult.success();
			}
			if (newTaxes > ConfigHandler.getNode("towny", "maxTaxes").getDouble())
			{
				src.sendMessage(Text.of(LanguageHandler.colorRed(), LanguageHandler.ERROR_TAXMAX.replaceAll("\\{AMOUNT\\}", String.valueOf(ConfigHandler.getNode("towny", "maxTaxes").getDouble()))));
				return CommandResult.success();
			}
			towny.setTaxes(newTaxes);
			DataHandler.saveTowny(towny.getUUID());
			src.sendMessage(Text.of(LanguageHandler.colorGreen(), LanguageHandler.SUCCESS_CHANGETAX));
		}
		else
		{
