...
public class TownyVisitExecutor implements CommandExecutor
{
	...
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException
	{
		if (src instanceof Player)
		{
			...
			{
				src.sendMessage(Text.of(TextColors.YELLOW, "/t visit <towny> [name]"));
			...
			if (clicker == Utils.CLICKER_NONE && !towny.getFlag("public")) {
				src.sendMessage(Text.of(TextColors.RED, LanguageHandler.ERROR_TOWNNOTPUBLIC));
				return CommandResult.success();
			}
			...
			if (!ctx.<String>getOne("name").isPresent())
			{
				src.sendMessage(Text.builder()
						...
						.append(Utils.formatTownySpawns(towny, TextColors.YELLOW, clicker))
